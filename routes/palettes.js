...
//유사 팔레트 추천(제작중)
//팔레트 태그 순서를 주요색 / 화려한(보색), 부드러운(유사색), 단정한(단색)/ 테마(없을 시 0으로 고정)으로 고정.
//이외의 것들은 뒤에 미루어둠
//두가지 모드, 색상과 조합 또는 테마로 구동할 수 있음
router.get('/recomm', async(req, res)=>{
    try{
        const palette = await Palette.findOne({_id:req.body.pid})
        const recommod = req.body.mod
        ...
        switch (recommod){
            case "color":
                tag1 = palette.tags[0]
                tag2 = palette.tags[1]
                result = await Palette.find({tags: [tag1, tag2], maker:0})
                break;

            case "theme":
                tag1 = palette.tags[2]
                result = await Palette.find({tags:tag1, maker:0})
                break;

            default:
                return res.status(400).json({error: 'recommend mod error'});
        }
        res.json({result});
    }
    catch(err){
...
//가장 최근에 저장한 팔레트를 기반으로 추천해주는 알고리즘
//주요색 - 조합 - 테마 순으로 태그가 있다는 가정 하에 셋 중 랜덤으로 보여줌
//만약 페이지에 ~~~한 팔레트를 더 확인해보시겠어요? 의 단어를 얻고 싶다면 tag를 가져오면 됨
router.get('/recent', auth, async(req, res)=>{
    try{
        const palette = await Palette.findOne({_id:req.user.rpid})
        let num, tag, result

        num = Math.floor((Math.random()*3))
        tag = palette.tags[num]
        result = await Palette.find({tags:tag})
